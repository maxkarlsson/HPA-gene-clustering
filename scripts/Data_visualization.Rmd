...
dataset_metadata <- read_csv("run_settings/20221004_settings.csv") %>% 
  filter(!dataset %in% dataset_excluded)

data_path_file <- "run_settings/20221020 all_datasets.csv"
data_paths <- read_csv(data_path_file)
...
cluster_annotation <-
  ...
  map(. %>% read_excel()) %>% 
  bind_rows(.id = "dataset_id") %>%
  filter(!dataset_id %in% dataset_excluded) %>% 
  rename(dataset_id = 1,
         cluster = 2,
         n_genes = 3,
         ann_specificity = 5, 
...
raw_data <-
  readRDS(paste0("data/processed/", 
                 str_remove(data_path_file, ".*/") %>% 
...
consensus_data <-
  ...
  {.[which(!str_detect(names(.), paste(dataset_excluded, collapse = "|")))]} %>% 
...
gene_classification <- 
  ...
  filter(!grepl(paste(dataset_excluded, collapse = "|"), id)) %>% 
  group_by(id) %>% 
  do({
    g_data <<- .
    
    get_specificity_db(enrichment_settings, g_data$id) %>% 
      select(-ts_score, -enhanced_score)
...
#   # mutate(tissue = str_remove(assay_name, "_.*")) %>% 
#   left_join(sc_cluster_annotation %>% 
#               select(1, 2, tissue_name, cluster_id, cell_type_name)) %>% 
#   filter(assay_name %in% sc_cluster_annotation$assay_name)
# 
# plot_data %>% 
#   select(cell_type) %>% 
#   distinct() %>% 
#   filter(!cell_type %in% names(consensus_palette))
# 
# plot <-
#   plot_data %>%
#   gather(type, coord, x, y) %>% 
#   group_by(assay_name) %>% 
#   mutate(coord = scales::rescale(coord, 0:1)) %>% 
#   ungroup() %>% 
#   spread(type, coord) %>% 
#   # filter(row_number() %% 15 == 0) %>%
#   # filter(assay_name == unique(assay_name)[1:2]) %>% 
#   # filter(assay_id %in% 1:2) %>%
#   ggplot(aes(x, y, fill = cell_type_name)) +
...
#         axis.title = element_blank())
# ggsave(savepath("sc umaps rastr.pdf"),
#        plot = plot,
#        width = 8, height = 8)
# 
# 
...
#         axis.text = element_blank(),
#         axis.title = element_blank())
# ggsave(savepath("sc umaps rastr2.pdf"),
#        plot = plot,
#        width = 8, height = 8)
```
...
bubble_data <- 
  enrichment_data %>%
  filter(!dataset_id %in% dataset_excluded) %>% 
  filter(!test_type %in% paste0("specificity_", dataset_excluded)) %>% 
...
plot_data <- 
  gene_classification_long %>%
  filter(specificity_category %in% c("Tissue enhanced", "Group enriched", "Tissue enriched")) %>% 
...
p1 | p3
ggsave(savepath("color space sc umap.pdf"),
       width = 7, height = 5)
...
ORA_settings <- 
  expand_grid(dataset_id = unique(ORA_data$dataset_id),
              db_dataset_id = unique(ORA_data$dataset_id)) %>% 
  filter(dataset_id != db_dataset_id)
...
ORA_net %>% 
  ...
  # scale_color_manual(values = dataset_palette) +
  scale_color_identity() +
  scale_fill_gradientn(colours = tableau_color_pal(palette = "Classic Cyclic")(13)) +
  theme_void()+
  coord_fixed()
...
ORA_net %>% 
  ...
  theme_void()+
  coord_fixed()
ggsave(savepath("cluster ORA network 2.pdf"),
       width = 8, height = 8)
...
ORA_net %>% 
  ...
  theme_void()+
  coord_fixed()
ggsave(savepath("cluster ORA network 3.pdf"),
       width = 8, height = 8)
...
plot_genes <- 
  c(47, 76, 31, 42, 57, 49, 24, 36, 51, 6) %>% 
  enframe("order", "cluster") %>% 
...
plot_data %>% 
  ...
  scale_data(max_scale = T) %>% 
  t() %>% 
  pheatmap(annotation_col = plot_genes %>% 
             select(gene, order) %>% 
             column_to_rownames("gene"),
           clustering_method = "ward.D2",
           show_colnames = F,
...
plot_data %>% 
  ...
  left_join(plot_genes %>% 
            ...
            by = c("ensg_id" = "gene")) %>% 
  arrange(order) %>% 
  cluster_long_data_grouped(group_col = "order") %>% 
  mutate(cluster = factor(cluster, unique(cluster))) %>% 
  ggplot(aes(ensg_id, cell_type, fill = ntpm)) +
  geom_tile() +
  facet_grid(~cluster,
             space = "free",
             scales = "free_x") +
  # scale_fill_viridis_c(option = 2) +
  scale_fill_gradient(low = "gray95", high = "skyblue3") +
  theme_bw() +
  theme(panel.spacing = unit(0,"mm"),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        panel.border = element_rect(color = NA))
ggsave(savepath("sperm heatmap.pdf"),
       width = 7, height = 3)
...
plot_data %>% 
  ...
  group_by(ensg_id) %>% 
  # mutate(ntpm = ntpm / max(ntpm)) %>% 
  mutate(ntpm = (ntpm - mean(ntpm)) / sd(ntpm)) %>% 
  ungroup() %>%  
  left_join(cluster_annotation) %>% 
  arrange(order) %>% 
  mutate(cluster = factor(cluster, unique(cluster)),
         cell_type = factor(cell_type, c("Spermatogonia", "Spermatocytes",
                                         "Early spermatids", "Late spermatids"))) %>%
  ggplot(aes(cell_type, ntpm, group = ensg_id)) +
...
ggsave(savepath("sperm clusters exp distr.pdf"),
       width = 6, height = 12)

IHC_data %>% 
  janitor::clean_names() %>% 
  filter(str_detect(cell_type, "sperm")) %>% 
  inner_join(plot_genes) %>% 
  arrange(order) %>% 
  mutate(cluster = factor(cluster, unique(cluster)),
         detected = level != "Not detected") %>% 
  group_by(cluster, cell_type, detected) %>% 
  count() %>% 
  ggplot(aes(cluster, n, fill = cell_type)) +
...
IHC_data %>% 
  ...
  group_by(cluster, cell_type, level) %>% 
  count() %>% 
  ggplot(aes(cluster, n, fill = level)) +
  geom_col() +
  facet_grid(level~cell_type)
  ...
  mutate(cluster = factor(cluster, unique(cluster)),
         ...
         level = factor(level, 
                        c( "Not detected",
                           ...
                           "Medium",
                           "High"))) %>% 
  group_by(cell_type, level, cluster) %>% 
  count() %>% 
  group_by(cluster) %>% 
  mutate(frac = n / sum(n)) %>% 
  ggplot(aes(cell_type, level, size = frac)) +
...
IHC_data %>% 
  ...
  mutate(frac = n / sum(n)) %>% 
  ggplot(aes(cell_type, level, fill = frac)) +
  # geom_line(aes(group = gene)) +
  geom_tile() +
  facet_grid(~cluster) +
  coord_fixed() +
  ...
  mutate(cluster = factor(cluster, unique(cluster)),
         ...
         level = factor(level, 
                        c( "Not detected",
                           ...
                           "Medium",
                           "High")),
         level_norm = unclass(level) / max(unclass(level))) 

plot_data2 %>% 
  group_by(cell_type, level, cluster) %>% 
  count() %>% 
  ggplot(aes(cell_type, level, size = n)) +
  # geom_line(aes(group = gene)) +
  geom_point() +
  facet_grid(~cluster) +
  coord_fixed() +
...
plot_data2 %>% 
  ggplot(aes(cell_type, level_norm)) +
  geom_path(aes(group = gene),
            alpha = 0.1) +
  geom_point(data = . %>% 
               group_by(cell_type, level_norm, cluster) %>% 
               count(),
             aes(size = n)) +
  facet_grid(~cluster) +
  coord_fixed(5) +
...
#   mutate(ntpm = (ntpm - mean(ntpm)) / sd(ntpm)) %>% 
#   ungroup() %>%  
#   filter(str_detect(cell_type, "Sperm|sperm"),
#          ensg_id %in% plot_genes$gene) %>%
#   left_join(cluster_annotation) %>% 
#   arrange(order) %>% 
#   mutate(cluster = factor(cluster, unique(cluster)),
#          cell_type = factor(cell_type, c("Spermatogonia", "Spermatocytes",
...
database_terms <- 
  read_tsv("data/processed/20221027 database terms.tsv")


filtered_terms <- 
  database_terms %>% 
  mutate(ensg_id = factor(ensg_id, unique(ensg_id))) %>% 
  filter(id %in% c("GO_BP_original",
                   ...
                   "reactome",
                   "trrust")) %>% 
  group_by(ensg_id, .drop = F) %>% 
  summarise(n = n_distinct(term),
            terms = paste(sort(term), collapse = ";"),
            n_db = n_distinct(id),
            dbs = paste(sort(id), collapse = ";")) %>% 
  arrange(n) %>% 
  left_join(gene_info %>% 
              select(ensg_id, gene_name, gene_description)) %>% 
  filter(ensg_id %in% unique(clustering_data$gene))
...
filtered_terms %>% 
  ...
  count() %T>%
  print(n = 100) %>% 
  ggplot(aes(n, nn, fill = n_db)) +
  geom_col(width = 1) +
  scale_x_continuous(limits = c(-1, 50)) +
  scale_fill_continuous(name = "#Databases") +
...
plot_data <- 
  ...
  left_join(clustering_data,
            by = c("ensg_id" = "gene")) %>% 
  left_join(cluster_annotation %>% 
              select(1, 2, 3, 5, 6, reliability)) 

plot_data %>% 
  group_by(dataset_id, cluster, n_genes, ann_specificity, ann_function, reliability) %>% 
  count() %>% 
  mutate(frac = n / n_genes) %>% 
  arrange(-frac) %>% 
  print(n = 50)


plot_data %>% 
  filter(dataset_id == "tissue", cluster == 20) %>% 
  print(n = 100)

#Cilia
plot_data %>% 
  filter(dataset_id == "tissue", cluster == 49) %>% 
  print(n = 100)

plot_data %>% 
  filter(dataset_id == "tissue", cluster == 42) %>% 
  print(n = 100)

plot_data %>% 
  filter(dataset_id == "singlecell", cluster == 56) %>% 
  print(n = 100)

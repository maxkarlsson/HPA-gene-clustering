...
if(file.exists("data/processed/all_cluster_data_mockup.rds")) {
  all_data <- 
    readRDS("data/processed/all_cluster_data_mockup.rds")
} else {
  all_data <- 
...
dataset_metadata <- 
  ...
  separate(col = dataset_id, into = c("dataset", "type"), remove = F, sep = "_") %>%
  mutate(type = ifelse(dataset_id %in% c("tissue", "blood_consensus"),
                       "consensus",
                       "sample")) %>% 
  select(-i)
...
if(file.exists("data/processed/all_cluster_data_metadata_mockup.rds")) {
  
  all_data_meta <- 
    readRDS("data/processed/all_cluster_data_metadata_mockup.rds")
  ...
  all_data_meta <-
    ...
    mutate(sample = case_when(dataset == "singlecell" ~ cell_type,
                              type == "sample" ~ sample, 
                              type == "consensus" ~ sample_id)) %>% 
    select(dataset_id, dataset, type, sample_id, sample)
  
  saveRDS(all_data_meta, "data/processed/all_cluster_data_metadata_mockup.rds")
}
...
#   filter(grepl(run_id, filename)) %>%
#   filter(grepl("neighbors", filename)) %>%
#   filter(!grepl("log", filename)) %>%
#   mutate(dataset_id = gsub(paste0("data/processed/", run_id, "_|_neighbors_.*$"), "", filename)) %$%
#   set_names(filename, dataset_id) %>%
#   lapply(function(filename_) readRDS(filename_))
# # 
# # # Distance files are very big, so just read when used: (MODIFIED)
# all_data_dists_filenames <-
#   list.files("data/processed/", full.names = T) %>%
#   enframe("i", "filename") %>%
#   filter(grepl(run_id, filename)) %>%
#   filter(grepl("_dist_", filename)) %>%
#   filter(!grepl("log", filename)) %>%
#   mutate(dataset_id = gsub(paste0("data/processed/",run_id, "_|_dist*.*$"), "", filename)) %$%
#   set_names(filename, dataset_id)


```

## Selection of informative PCs
```{r}
# Read PCA data
...
all_data_pca <- 
  ...
  enframe("i", "filename") %>% 
  filter(grepl(run_id, filename)) %>% 
  filter(grepl("pca", filename)) %>% 
  mutate(dataset_id = gsub(paste0(".*", run_id, "_|_pca_.*$"), "", filename)) %$% 
  set_names(filename, dataset_id) %>%
  lapply(function(filename_) readRDS(filename_))

# Retrieve number of informative PCs

informative_pc <-
  all_data_pca %>% 
  sapply(function(pca)
    pc <- pca$sdev[pca$sdev >= 1] %>% tail(1) %>% enframe() %$% name) 


met <- dataset_metadata %>% 
  left_join(informative_pc %>% 
              enframe %>% 
              rename(PC = value) %>%  
              mutate(PC = as.numeric(gsub("PC", "", PC))) , by = c("dataset_id" = "name"))

met %>% 
  pull(dataset_id) %>% 
  map(~ all_data_pca[[.x]]$sdev %>% 
        enframe() %>% 
        rename(PC = name, sd = value) %>%  
        mutate(PC = as.numeric(gsub("PC", "", PC))) %>% 
        ggplot(aes(PC,sd)) + 
        geom_point() +
        geom_line() +
        ggtitle(paste0("PCA ", .x)) + 
        geom_vline(xintercept = met %>%  filter(dataset_id == id) %>%  pull(PC), linetype="dotted") +
        theme_bw())
...
met %>% 
  ...
  theme_bw() +
  geom_vline(data=met, aes(xintercept = PC), linetype = "dashed")
...
ARI_res_2 <- 
  ...
  do({
    dat <<- .
    combn(1:max(dat$seed), m=2, function(x)   
                 ARI(dat %>% filter(seed == x[1]) %>% select(gene,cluster) %>%  to_cluster(), 
                           dat %>%  filter(seed == x[2]) %>% select(gene,cluster) %>%  to_cluster())) %>% 
      as_tibble()
  })
...
ARI_res_2 <- 
  ...
  do({
    ...
    combn(1:max(dat$seed), m=2) %>% 
      ...
      group_by_all() %>% 
      mutate(ARI = ARI(dat %>% filter(seed == seed1) %>% select(gene,cluster) %>%  to_cluster(), 
                       dat %>%  filter(seed == seed2) %>% select(gene,cluster) %>%  to_cluster()))
...
ARI_res_2 %>% 
  ...
  geom_point() +
  geom_smooth() +  facet_grid(metric ~ dataset_id, scales = "free_y") +
  theme_bw() +
  ggtitle("Clustering evaluation")
...
ARI_res %>% 
 # mutate(seed = c(1:10)) %>% 
  rename(ARI = value) %>% 
  left_join(all_data_cluster_connectivity) %>% 
  left_join(all_data_cluster_MI %>%  select(-MI, -mean, -sd)) %>%
  gather(metric, value, ARI, connectivity, avg_silhouette_width, zscore) %>% 
  left_join(all_data_cluster %>% 
              group_by(dataset_id,resolution, seed) %>% 
              mutate(k = max(cluster) + 1) %>% 
              select(-gene,-cluster) %>% 
  ...
  ggplot(aes(k, value, color = as.factor(resolution))) +  
  geom_point() +
  geom_smooth(inherit.aes = F, aes(k, value), color = "darkgrey") +
  facet_grid(metric ~ dataset_id, scales = "free_y") +
  theme_bw() +
  ggtitle("Clustering evaluation")
...
# Consensus clustering (clue package) - TEST (for 1 data set, 1 resolution)

clusters <- all_data_cluster %>% 
  filter(dataset_id == "blood", resolution == 4) %>% 
  unite(resolution_seed, dataset_id, resolution, seed)  %>% 
  group_split(resolution_seed) %>% 
  map(to_cluster)

partitions <- clusters %>%  map(~as.cl_partition(.x))

cons_clustering <- cl_consensus(partitions, method = "SE")

final_clustering <- cl_class_ids(cons_clustering)

final_clustering_df <- data.frame(gene = names(final_clustering), 
                                  cluster = as.numeric(final_clustering))
...
all_graph_umap_data %>% 
  ...
  coord_fixed() +
  theme_bw() + 
  scale_color_manual(values = cluster_palette50)
...
## María's functions not yet implemented

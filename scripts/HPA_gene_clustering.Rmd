...
dataset_palette <- c("tissue" = "#0083C4",  "brain" = "#FFDD00", "blood" = "#CF161A", celline = "#97CF16") #"singlecell" = "#6AA692",
...
# ARI_blood <- read_tsv("results/Clustering results/blood testv4/evaluation/ARI.tsv")
# 
# ARI_blood %>% 
#   filter(resolution == "1.7") %>% 
...
tissue <- all_clusterings %>% filter(dataset_id == "tissue",
                                          ...
                                          k < 100)
# %>% 
#   filter((dataset_id == "blood" & (k > 30 | k < 60)) |
#            (dataset_id == "brain" & (k > 40 | k < 80)) |
#            (dataset_id == "celline" & (k > 40 | k < 70)) |
#            (dataset_id == "tissue" & (k > 60 | k < 90)) )
  ...
  do({
    ...
    if(file.exists(savefile_UMAP_hulls)) {
      ...
      hulls <- UMAP_hulls$hulls %>% 
        ...
        select(-density_type)
      write_tsv(hulls, savefile_UMAP_hulls)
    }
  }) %>% 
  ungroup() %>% 
  collect()
...
# Functional semantic space plots
```{r}

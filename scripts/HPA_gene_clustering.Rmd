...
#   left_join(final_clusterings ) %>% 
#   filter(dataset_id == "blood") %>% 
#   ggplot(aes(UMAP_1_scaled, UMAP_2_scaled, color = as.factor(cluster))) +
#   geom_point() +
#   coord_fixed() +
#   theme_void() +
#   geom_point(data = UMAP_hulls$center_density, aes(x,y), inherit.aes = F) 
...
old <- read_tsv("results/Clustering_results/tissue_HPA21v2/UMAP/old/cluster_centers.tsv")
new <- read_tsv("results/Clustering_results/tissue_HPA21v2/UMAP/cluster_centers.tsv")

old %>% 
  full_join(new %>% 
              rename(x_2 = x,
                     y_2 = y)) %>% 
  mutate(a = (x-x_2)+(y-y_2)) %>% 
  filter(a != 0)

...
all_data_cluster <- 
  ...
  do({
    ...
    savefile_cluster <- 
      paste(file_structure[[g_dataset_id]]$clustering, "screening_clustering.csv", sep = "/")
    ...
    if(file.exists(savefile_cluster)) {
      ...
      cluster_res <-
        HPA_gene_clustering(g_data,
                            ...
                            seed = 1:10,
                            resolution = seq.int(0.1, 13, by = 0.1),
                            npcs = 100,
                            log_transform = F) 
...
consensus_test_3 %>% 
  group_by(dataset_id, resolution) %>% 
  mutate(max_k = max(cluster_cons),
         k = n_distinct(cluster_cons)) %>% 
  ungroup() %>% 
  mutate(new = max_k-k) %>% 
...
consensus_test_2 %>%
  group_by(dataset_id, resolution, cluster_cons) %>% 
...
to_plot <- 
  ...
  ungroup() %>% 
  group_by(dataset_id, resolution, seed, cluster) %>%
  mutate(n_genes = n_distinct(gene))  %>% 
  select(-gene) %>% 
  distinct() %>% 
  ungroup() %>% 
  group_by(dataset_id, resolution) %>% 
  mutate(min_size = min(n_genes),
...
to_plot %>%
  select(-seed, -cluster) %>% 
  distinct() %>% 
  gather(metric, value, min_size:avg_size) %>% 
...
graph_umaps <- 
  ...
  pblapply(function(dataset_id_) {
    ...
    savefile_UMAP <- 
      paste(file_structure[[dataset_id_]]$UMAP, "UMAP.csv", sep = "/")
    
    if(file.exists(savefile_UMAP)) {
      UMAP_res <-
        read_csv(savefile_UMAP)
      ...
      write_csv(UMAP_res, savefile_UMAP)
    }
...
ARI_res <- 
  ...
  do({
    ...
    savefile_ARI <- 
      paste(file_structure[[g_dataset_id]]$evaluation, "ARI.csv", sep = "/")
    
    if(file.exists(savefile_ARI)) {
      ARI_scores <-
        read_csv(savefile_ARI)
      ...
      write_csv(ARI_scores, savefile_ARI)
    }
...
MI_res_consensus <-
  ...
  do({
    ...
    savefile_MIGO <- 
      paste(file_structure[[g_dataset_id]]$evaluation, "MI_GO.csv", sep = "/")
    
    if(file.exists(savefile_MIGO)) {
      MI_scores <-
        read_csv(savefile_MIGO)
      ...
      write_csv(MI_scores, savefile_MIGO)
    }
...
seeds_clusters <-
  ...
  ungroup() %>% 
  group_by(dataset_id,resolution, seed) %>% 
  summarize(min_size = min(n_genes), 
            max_size = max(n_genes),
            avg_size = mean(n_genes))
...
seeds_clusters %>% 
  ...
  ggplot(aes(x = resolution, y = value, color = metric), size = 0.01) +
  geom_point() +
  geom_smooth() +
  theme_bw() +
...
```{r}
tissue_scaled_clusterings <-
  umap_scaled %>% 
  filter(dataset_id == "tissue") %>% 
  left_join(tissue_clusters_consensus)

pdf("umaps4.pdf")

plots <- 
  lapply(unique(tissue_clusters_consensus$resolution), function(r) {
       tissue_scaled_clusterings %>% 
         filter(resolution == r) %>% 
      arrange(weighted_dist) %>% 
      mutate(cluster = factor(cluster, unique(cluster))) %>% 
       ggplot(aes(x = UMAP_1, y = UMAP_2, color = cluster)) +
         geom_hex(data = umaps_scaled %>% filter(dataset_id == "tissue"), 
...
```{r}
## Checkpoint
all_data_cluster_consensus %>% 
  group_by(dataset_id) %>% 
  mutate(num_clusters = n_distinct(cluster),
         max_cluster = max(cluster),
         n_genes = n_distinct(gene)) %>%
  group_by(dataset_id,cluster) %>% 
  mutate(size = n_distinct(gene)/n_genes)%>% 
  ungroup() %>% 
  group_by(dataset_id) %>% 
  summarise(avg_size = mean(size),
            ...
            max_cluster) %>% 
  distinct() %>% 
  gather(metric, value, avg_size, min_size, num_clusters, max_cluster) %>% 
...
ARI_res %>% 
  ...
  summarise(ARI_mean = mean(ARI),
            ...
            CV = ARI_sd / ARI_mean) %>% 
  ungroup() %>% 
  select(-ARI_mean, -ARI_sd) %>% 
  left_join(MI_res_consensus %>% 
              select(-MI,-mean_MI, -sd_MI) %>% 
              group_by(dataset_id, resolution) %>% 
              spread(key=GO_domain, value = z_score) %>% 
              summarize(MI_z_score = mean(biological_process, 
                                          cellular_component,
                                          molecular_function))) %>% 
  left_join(eval_gene_distances %>% 
              select(dataset_id,resolution, avg_weighted_dist) %>% 
              unique()) %>% 
  inner_join(filt_res) %>% 
  gather(metric, value, CV:avg_weighted_dist) %>% 
  ungroup() %>% 
  group_by(dataset_id, resolution, metric) %>% 
#  mutate(scaled_value = scales::rescale(value,c(0,1))) %>% 
  ggplot(aes(resolution, value, group = metric)) +
  geom_point() +
  geom_smooth() +
...
final_clusterings_consensus %>% 
  filter(dataset_id == "tissue") %>% 
  group_by(dataset_id, cluster_cons) %>% 
  mutate(size = n_distinct(gene)) %>% 
  select(-gene) %>% 
  distinct() %>% 
  ggplot(aes(x = cluster_cons, y = size)) +
  geom_bar(stat = "identity") #+
#  facet_wrap(~dataset_id)

final_clusterings_consensus %>%
  group_by(dataset_id) %>% 
  mutate(max_k = max(cluster),
   k = n_distinct(cluster)) %>% 
  ungroup() %>% 
  mutate(a = max_k-k) %>% 
  pull(a) %>%  

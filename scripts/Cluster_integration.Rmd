...
# Clustering overview 
## Clustering UMAPs (no annotation)
```{r}
cluster_color_dict <-
  clustering_data %>% 
...
umap_data %>% 
  ...
  geom_polygon(data = umap_polygons %>% 
               ...
               #show.legend = F, 
               alpha = 0.9)+ 
  scale_fill_manual(values = specificity_pal %>% deframe()) +
  geom_text(data = umap_centers %>% 
              mutate(cluster = as.character(cluster)) %>% 
              left_join(cluster_annotation) %>% 
              filter(dataset_id == "tissue"),
            aes(x, y, label = Function), #, color = Specificity
            inherit.aes = F,
            show.legend = F) +
    scale_color_manual(values = specificity_pal %>% deframe() ) 

ggsave(savepath("tissue_color_specificity_legend_black.pdf"))



### More transparent (?) - Specificities
...
ggsave(savepath("tissue_color_specificity_nofunction.pdf"))
...
low_clust <- 
  ...
  select(Function, full_annotation, x, y)
...
clustering_data %>% 
  ...
  geom_scatterpie(aes(x=x, y=y, group=cluster, r = 0.03), 
                  ...
                  color=NA, alpha=.9) +
  scale_fill_manual(values = gene_category_pal)  +
  geom_text(data = low_clust, aes(x, y, label = full_annotation)) #full_annotation
...
heatmap_data_scaled <- expression_data_scaled
...
# some_gemes <- 
#   tau_scores %>% 
#   filter(!is.finite(tau_score),
#          dataset_id == "singlecell_consensus")
# expression_data$singlecell_sample %>% 
#   filter(ensg_id %in% some_gemes$gene) %>% 
...
# 
# gene_classification %>% 
#   select(id, gene =ensg_id, specificity_category) %>% 
#   mutate(id = gsub("specificity_", "", id)) %>% 
#   left_join(tau_scores %>% 
#               mutate(id = dataset_id) %>% 
#               mutate(id = gsub("_sample", "", id))%>% 
#               mutate(id = gsub("_consensus", "", id))%>% 
#               mutate(id = gsub("_region", "", id))) %>% 
#   subset(is.na(tau_score)) %>%
#   View
#   pull(specificity_category) %>% 
#   unique()
# 
# 
# gene_classification %>% 
...
#   group_by(cluster) %>% 
#   summarise(n_genes = n_distinct(gene) / 10000) #%>% distinct()
# 
# #  summarise(n_genes = n_distinct(gene) / n) %>% distinct()
# 

ggsave(savepath("scatterpie_singlecell.pdf"))
...
heatmap_data_scaled_cluster %>% 
  ...
  do({
    ...
    plot_cluster <- 
      plot_cluster_1 / plot_cluster_2 / plot_cluster_3 +
      plot_layout(heights = c(0.25,1.5,1.5))
    
    
    plot_dendrogram / heat / plot_cluster / sp_plot / plot_tau +
      plot_layout(heights = c(1,9,4, 2, 2), nrow = 5, 
                  guides = 'collect' )
    
      
  ggsave(savepath(paste(g_dataset_id, "heatmap_v3.pdf", sep = "_")))  
...
ggsave(savepath("ORA_gen_classes.pdf"))

...
corrplot(cor_matrix,
  method = "shade",
  type = "upper",
  diag = F, # show only upper side,
  hclust.method ="ward",
  pch.col = "black"
)

  hclust.method = c("complete", "ward",col <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
...
rquery.cormat(cor_matrix, type="upper")
...
d %>% 
  ggplot(aes(x=connectivity_index,y=MI_zscore)) +
  geom_point()
library("ggpubr")


ggscatter(d, x = "avg_silhouette_width", y = c("connectivity_index","MI_zscore","annotability"),
          combine = T,
...
clusterings_umap %>% 
  group_by(features) %>% 
  mutate(scale_dist = paste(strsplit(features, " ")[[1]][1], strsplit(features, " ")[[1]][2]),
...
library(colorspace)
ann_transp <- unlist(map(ann_colors$clust, ~adjust_transparency(.x, alpha = 0.02)))
...
clusterings_umap %>% 
  ...
  mutate(scale_dist = paste(strsplit(features, " ")[[1]][1], strsplit(features, " ")[[1]][2]),
                            clust = strsplit(features, " ")[[1]][3],
                            k = as.numeric(strsplit(features, " ")[[1]][4])) %>% 
           ggplot(aes(x=V1, y=V2, color = clust, fill = clust)) +
           geom_point( size=3) +
  theme_minimal() +
  geom_encircle() +
  scale_color_manual(values = clust) +
...
```{r Visualizations final clustering}
final_clustering <-
  all_partitions %>% 
  filter(id=="zscore pearson medoids 50")

umap_zscores %>% umap_plot(color_by="cluster",color_groups = final_clustering )
...
annotation <- read_excel(path = "20210426_annotation.xlsx", sheet = "cluster_info")
...
r35 <- annotation_report(35)

pdf("8.pdf", width = 7, height = 9)
r8
dev.off()
...
pdf("reports_v3.pdf", width = 7, height = 9)
reports_v5
dev.off()
...
data <- clustering %>% 
  left_join(ortholog_info, by = c("gene" = "enssscg_id")) %>% 
  select(ID = ensg_id, value) %>% 
  na.omit() %>% 
  left_join(GO_filtered) %>% 
  na.omit()

all_GO <- colnames(GO_sparse)[c(-1,-2)]
...
GO_sparse <-
  ...
  group_by(cluster) %>% 
  summarise(across(head(all_GO,1):tail(all_GO,1), sum)) %>% 
  as_tibble()
...
GO_umap <- 
  ...
  select(c(1:40,"cluster")) %>% 
  umap_calc(n_neigh = 15, row_names = "cluster")

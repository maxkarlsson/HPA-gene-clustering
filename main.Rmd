...
```{r Evaluation based on Mutual Information (ClusterJudge)}
...
test %>%
 # select(-annotability) %>%
      mutate(MI_norm = MI/MI_random) %>% 
  mutate(MI_zscore = (MI - MI_random)/sqrt(MI_random_var)) %>%

  ggplot(aes(x=id, y = MI_zscore, fill = (k))) +
  geom_bar(stat = "identity")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
...
all %>%
  spread(key=measure, value=value) %>%
  ungroup() %>%
  mutate(rank_connectivity= rank(connectivity_index,ties.method = "random")) %>%
mutate(        rank_asw = rank(-(avg_silhouette_width),ties.method = "random"),
         rank_MI = rank(-(MI_zscore),ties.method = "random"),
         rank_annotability = rank(-(annotability),ties.method = "random")) %>%
  column_to_rownames("id") %>%
  select(rank_connectivity,rank_asw,rank_MI,rank_annotability) %>%
  pheatmap(cluster_cols = F, fontsize = 8, border_color = NA, 
           ...
           annotation_row  = annotation,
            show_rownames = F)
           #annotation_colors = list(values =viridis(24,option = "A")))


# Select best k for each clustering
...
best_k_of_each <- 
  ...
  do({
    data <- .
    best <- data %>% find_bestk(measures = measures)
    data %>% filter (id == best)
  })
...
all_best_ranked <- 
  ...
  ungroup() %>%
  mutate(rank_connectivity= rank(connectivity_index,ties.method = "random"),
         rank_asw = rank(-(avg_silhouette_width),ties.method = "random"),
         rank_MI = rank(-(MI_zscore),ties.method = "random"),
         rank_annotability = rank(-(annotability),ties.method = "random"))

best_k_of_each %>% find_bestk(measures = measures, n=24)
...
#2. max pearson medoids 50
#3. max pearson fasthclust 50



best_of_each_clustering <- 
  best_k_of_each %>%
  group_by(id) %>%
  mutate(clustering = strsplit(scale_dist_clust, " ")[[1]][3]) %>%
  ungroup() %>%
  group_by(clustering)  %>%
  do({
    data <- .
    best <- data %>% find_bestk(measures = measures, n= 1)
    data %>% filter (id == best)
  })

best_of_each_clustering %>%
 # ungroup() %>%
  find_bestk(measures = measures, n=4)

annotation_2 <- data.frame(names =
  best_k_of_each %>%
  find_bestk(measures = measures, n = 24),
  values = (as.numeric(c(1:24)))) %>%
...
all_best_ranked %>%
  column_to_rownames("id") %>%
  select(rank_connectivity,rank_asw,rank_MI,rank_annotability) %>%
  # select(rank_connectivity,rank_asw,rank_BHI) %>%
  # select(rank_connectivity,rank_asw,rank_MI) %>%
  #select(rank_connectivity,rank_asw,rank_annotability,rank_min_significance) %>%
    #select(rank_connectivity,rank_asw,rank_cor) %>%
  pheatmap(cluster_cols = F, fontsize = 8, border_color = NA, 
...
all %>%
  group_by(id) %>%
  mutate(clustering = strsplit(id," ")[[1]][3]) %>% 
  select(id, clustering, measure, value) %>%
  ungroup() %>% 
  filter(measure == "time") %>% 
...
```{r Normal UMAP correlation UMAP}
#1. zscore pearson louvain 68
partition <-
  all_partitions %>%
...
clustering_umaps <- function(id, umaps, partitions) { #, 
  ...
  p <- umap_plot(umap_res = umap, color_by = "cluster", color_groups = partition) 
  
  return(p)
}


c <- as.list(unique(all_partitions$id) %>% head(3))
...
clustering_umaps(id = "zscore pearson leiden 73", umaps = umaps, partitions = all_partitions) +
  ggtitle("zscore pearson leiden 73")
...
#min-max? can be negative?

#min((to_model %>% filter(measure == "annotability"))$value_scaled)
...
anova_model %>% 
  ...
  tidy() %>% 
  arrange(adj.p.value)
# add final ranking as metric to combine
...
# Single model
model <- lm(value ~ scaling + distance + clustering + k, 
            data = to_model %>% filter(measure=="MI_zscore")) 

anova(model)

models %>% 
  group_by(measure) %>%
  summarise(sum(`Sum Sq`))
...
ids <- unique(all_partitions$id)

for (id in ids) {
    data <- all_partitions %>% filter(id == id)
    id <- unique(data$id)
    if(grepl("zscore",id)) {
      umap <- umaps[[1]]
    }
    ...
    p <-  umap_plot(umap_res = umap, color_by = "cluster", color_groups = data) +
      ggtitle(id)
    plots <- c(plots,p)
}


pdf("umaps_all_partitions.pdf")
plots
dev.off()

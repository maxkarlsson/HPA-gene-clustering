...
load("data/processed/evaluation.Rdata")

random <- random %>% mutate (id = "random")

df <- bind_rows(df,random)
...
rownames(res_ari) <- c(clusterings,"random")
colnames(res_ari) <- c(clusterings,"random")
...
ev %>%
  ...
  mutate(connectivity = scale(connectivity)) %>%
  pheatmap(cluster_cols = F)
  
```

```{Visual evaluation - UMAPs}
...
to_pairwise <- df %>% spread(key = id, value = cluster) %>%
  select(-gene) %>% as_tibble()


options <- as.list(colnames(to_pairwise))
xy <- combn(options, 2)

xy[,2][1]


ARI(clust_leiden[[1]]$cluster$value,clust_leiden[[9]]$cluster$value)
ARI(clust_leiden[[1]]$cluster$value,clust_leiden[[8]]$cluster$value)
...
clus_umap <- pcas[[1]]$scores %>%
  ...
  select((1:30)) %>%
  rownames_to_column("enssscg_id") %>%
  umap_calc(row_names = "enssscg_id", n_neigh = 15, n_comp = 2, color_tissue = T)
...
df_all <- df %>% filter(id %in% ids) %>% group_by(id,cluster)  

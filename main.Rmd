...
enrichment_140 <- enrich(clustering = clustering, orthologs = ortholog_info)
enrichment_random <- enrich(clustering = clustering_random, orthologs = ortholog_info)

enrich_cluster(clustering=clustering, num = 0, orthologs = ortholog_info )
...
mean(enrichment_131$enriched_terms$qvalue)

enrichment_140$enriched_terms %>%
  filter(qvalue < 0.05) %>%
  pull(qvalue) %>%
  length()

enrichments <-
  list(enrichment_random$enriched_terms,
     enrichment_66$enriched_terms,
     enrichment_88$enriched_terms,
     enrichment_104$enriched_terms,
     enrichment_119$enriched_terms,
     enrichment_131$enriched_terms,
     enrichment_140$enriched_terms)



significance_overall <- map(enrichments, ~.x  %>% pull(qvalue) %>% mean())
significance_filtered <- map(enrichments, ~.x %>% filter(qvalue < 0.05) %>% pull(qvalue) %>% mean())
significance_overall_perclust <- map(enrichments, ~.x  %>% group_by(Cluster) 
                                      %>% summarize(mean(qvalue)) %>% pull(`mean(qvalue)`) %>%
                                       mean())
significance_filtered_perclust <- map(enrichments, ~.x %>% filter(qvalue < 0.05)  %>%
                                        group_by(Cluster) %>% summarize(mean(qvalue)) %>%
                                        pull(`mean(qvalue)`) %>% mean())

num_sig_terms_total <- map(enrichments, ~.x %>% filter(qvalue < 0.05) %>% pull(qvalue) %>% length)
num_sig_terms_per_enriched_cluster <- map(enrichments, ~.x %>% 
                                            filter(qvalue < 0.05) %>% group_by(Cluster) %>%
                                            summarize(length(qvalue)) %>% pull(`length(qvalue)`) %>%
                                            mean())

avg_minq_cluster <- map(enrichments, ~.x %>% group_by(Cluster) %>% 
                  summarize(min(qvalue)) %>% pull(`min(qvalue)`) %>% mean() )

perc_enrichmemt <- map(enrichments, ~
...
clusterings <- c("random", "louvain_66", "louvain_88", "louvain_104", "louvain_119", "louvain_131", "louvain_140")
df <- data.frame(clusterings = clusterings, 
...
df2 <-
  df %>%
  gather(key = measure, value = value, significance_overall:BHI) 

df2$clusterings <- factor(df2$clusterings,levels = unique(df2$clusterings))
df2$measure <- factor(df2$measure,levels = unique(df2$measure))
...
save(df, file ="data/processed/test_biological.Rdata")
...
load("data/processed/hclust_multk.Rdata")
clustering <- hclust_multk[[1]][[1]]$cluster
c<- 
  clustering_random %>%
  left_join(ortholog_info, by = c("gene" = "enssscg_id")) %>%
    na.omit() %>%
...
mutinfo_zscore <- function(clustering, orthologs, GO_terms) {
  ...
  zscore_df <- clusterJudge_z_score(clusters = clusters, entity.attribute = GO_terms, 
                                    nmb.randomizations = 100, plot.saveRDS.file=NULL)
